...
- **Enlaces que Cambian en Cada Carga**: Hay proveedores que no publican una dirección fija y sirven su instalador desde una CDN con un nombre distinto cada vez. Para esos, la ficha declara `download_page` y `download_link_pattern`: la dirección se lee de la página oficial **en el momento de descargar**, que es el enlace que obtendría quien lo hiciera a mano. Si la página no responde o cambia de forma, se recurre al `download_url` guardado en el catálogo y el diario anota por qué.
...
### 🔎 Búsqueda en Cuatro Catálogos a la Vez
Escribir en la barra superior pregunta a la vez al **catálogo de WinSlimCenter**, a la **Microsoft Store**, a **WinGet** y a **Chocolatey**. Cada origen responde en su propia subsección, en ese orden y separadas por una línea: primero lo revisado a mano, después lo que firma Microsoft y al final los dos repositorios completos. Ninguna espera a las demás, así que un origen lento no retrasa lo que las otras ya trajeron.

- **WinGet**: consulta `winget search` sobre su propio repositorio y lista nombre, identificador y versión. Instalar usa la misma maquinaria que el catálogo —cola, barra de progreso, cancelación y aviso final—, porque la ficha que se envía al backend es una entrada de catálogo efímera con `source_type: winget`.
- **Chocolatey**: consulta la API pública del repositorio de la comunidad (`community.chocolatey.org/api/v2`), de modo que **contesta aunque el equipo no tenga Chocolatey instalado**. Los paquetes sin moderar se marcan como tales y quedan detrás de los aprobados. La primera instalación instala Chocolatey con su script oficial y después el paquete; la tarjeta lo advierte antes de pulsar.
- **Chocolatey y los permisos**: instalar el gestor y sus paquetes escribe en `C:\ProgramData` y en el entorno de la máquina, así que todo lo suyo exige el token de administrador. La tienda lo tiene siempre —su manifiesto pide `requireAdministrator`—, y aun así se comprueba antes de empezar: si faltara, la sección lo dice y la operación se detiene con esa razón, en vez de dejar a Chocolatey a medias con un error de permisos. Cuando un paquete falla, lo que se muestra es su línea `ERROR:` —el servidor caído, el 404, el certificado inválido— y no el «See the log for details» con el que Chocolatey termina siempre.
- **Sin duplicados**: un paquete que el catálogo de WinSlimCenter ya cubre se descuenta de los resultados del repositorio, para no ofrecer dos veces el mismo programa con dos fichas distintas.
...
| **🎮 Juegos** | Steam, Epic Games Launcher, GOG GALAXY, EA App, Ubisoft Connect, Rockstar Games Launcher, Heroic Games Launcher, Cheat Engine, Mod Organizer 2, Lossless Scaling |
